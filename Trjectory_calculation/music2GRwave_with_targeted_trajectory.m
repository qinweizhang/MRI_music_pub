clear; close all; clc
if ispc
    cd('L:\basic\divi\Ima\parrec\Kerry\MRI_music\mp3')
else
    cd('')
end
filename = 'scannervoice.mp3';
...
% much
GR_wave_filt_export = GR_wave_filt(1:end,:); %<<<<<set export length here
...
dlmwrite(['L:\basic\divi\Ima\parrec\Kerry\MRI_music\dat\',fn,'[',num2str(duration),' min]_','music_GRwaveforms.dat'],GR_wave_filt_3ch,'delimiter','\t');

%% do something different with the profile



%put all to the positive, seems working
GR_wave_filt_export_2x = GR_wave_filt_export(:,1);
GR_wave_filt_export_2x = (GR_wave_filt_export_2x - min(GR_wave_filt_export_2x))/2; 
GR_wave_filt_export_2y = 0 * GR_wave_filt_export_2x;
GR_wave_filt_export_2z = 0 * GR_wave_filt_export_2x;

%random split in two channels
GR_wave_filt_export_2x = GR_wave_filt_export(:,1);
GR_wave_filt_export_2y = sin([1:length(GR_wave_filt_export_2x)]'/1000)*15;
GR_wave_filt_export_2x = GR_wave_filt_export_2x * 0; %GR_wave_filt_export_2x/2 - GR_wave_filt_export_2y; 
GR_wave_filt_export_2z = 0 * GR_wave_filt_export_2x;


%==============Export again =========================
GR_wave_filt_3ch = cat(2, GR_wave_filt_export_2x, GR_wave_filt_export_2y, GR_wave_filt_export_2z);


GR_wave_filt_3ch_scaled = real(round(GR_wave_filt_3ch * 1e6));  %GR_str_step is 1e-6
[~, fn, ~] = fileparts(filename);
dlmwrite(['L:\basic\divi\Ima\parrec\Kerry\MRI_music\dat\',fn,'[',num2str(duration),' min]_','music_GRwaveforms_2.dat'],GR_wave_filt_3ch_scaled,'delimiter','\t');


max_str_filt = max(GR_wave_filt_3ch,[],1);     %in mT/m
max_sr_filt = max(diff(cat(1,[0 0 0],GR_wave_filt_3ch),1)/slope_dur,[], 1);   %in mT/m/ms

disp('=========================music_GRwaveforms_2==================================')
str = sprintf('Sample points \t\t\t: %d ',length(y_filterd));
disp(str);
str = sprintf('Sample intervel \t\t: %f ms (%f KHz)',sample_interval_filt, (filter_bw * 1000)/1000);
disp(str);
...
str = sprintf('Scale factor \t\t\t: %f',scale);
disp(str);
str = sprintf('Max GR strength \t\t: %f (x) and %f (y) and %f (z) mT/m',max_str_filt);
disp(str);
str = sprintf('Max GR slewRate \t\t: %f (x) and %f (y) and %f (z) mT/m/ms',max_sr_filt);
disp(str);
disp('==============================================================')